...
class FormDetailMixin(object):

    def form_get(self, request, context):
        if context['form'].login_required and not request.user.is_authenticated():
...
class FormDetail(TemplateView, FormDetailMixin):
